...
async function authenticateProfileRequest(request: NextRequest) {
  ...
  if (authError || !authUser.user?.id) {
    console.error('[profile] Token verification failed:', authError);
    return { response: errorResponse(request, 'Invalid or expired bearer token.', 401) };
  }

  if (authUser.user.id !== userId) {
    console.warn(`[profile] User context mismatch. header=${userId}, token=${authUser.user.id}`);
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error(`[profile] Profile query error for user ${userId}:`, error);
      return errorResponse(request, error.message, 500);
    }

    const profile = normalizeProfile(data);

    console.info(`[profile] Profile retrieved for user ${userId}`);
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    console.info(`[profile] Profile updated for user ${userId}`);
